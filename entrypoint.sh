...
# sync base config
if [[ -d "$RUNNER_WORKSPACE/$APP/install" ]]; then
  echo "syncing from apps install folder"
  rsync -a $RUNNER_WORKSPACE/$APP/install/base $REPO_PATH/
  rsync -a $RUNNER_WORKSPACE/$APP/install/overlays $REPO_PATH/ 2>/dev/null
fi
